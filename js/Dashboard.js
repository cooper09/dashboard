...
function makeGraphs(error, apiData) {
	 console.log("makeGraphs: ", apiData)
//Start Transformations
	var dataSet = apiData;
	//var dateFormat = d3.timegi.format("%m/%d/%Y");
	dataSet.forEach(function(d) {
		console.log("this is our data: ", d.timestamp );
		//d.timestamp = dateFormat.parse(d.timestamp);
		console.log("formatted date: ", d.timestamp );
	//			d.timestamp.setDate(1);
		console.log("date posted: ", d.timestamp );
		// cooper s - something goes here...
	...
	//Define Dimensions
	var datePosted = ndx.dimension(function(d) { return d.item; });
	var gradeLevel = ndx.dimension(function(d) { return d.item; });
	var resourceType = ndx.dimension(function(d) { return d.color; });
	var fundingStatus = ndx.dimension(function(d) { return d.item; });
	var povertyLevel = ndx.dimension(function(d) { return d.color; });
	var state = ndx.dimension(function(d) { return d.description; });
	var totalDonations  = ndx.dimension(function(d) { return d.color; });
...
console.log("minimum date posted: " , minDate);
console.log("maximum date posted: " ,maxDate);
